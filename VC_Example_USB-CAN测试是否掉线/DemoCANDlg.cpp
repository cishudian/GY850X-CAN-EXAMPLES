...
/****************************************************************************/
    ...
    //调用动态链接库发送函数
	flag=VCI_Transmit(2,0,0,sendbuf);//CAN message send
	if(flag<1)
	{
		if(flag==-1)
...
UINT CDemoCANDlg::ReceiveThread(LPVOID v)
{
	...
	while(1)
	{
		...
		int Len=0;
		//调用动态链接看接收函数
			NumValue=VCI_Receive(2,0,0,pCanObj);
			//接收信息列表显示
			k++;
			CString strTime;
			SYSTEMTIME   systime;   
		...
		int ret=VCI_ConnectDevice(2,0);//请将这个连接设备的时间间隔设置长一些!!!!!!!
		if(ret==1)
			dlg->GetDlgItem(IDC_EDIT_DEVICE_CONNECT)->SetWindowText("正常通信");
		else if(ret==0)
		{
			dlg->GetDlgItem(IDC_EDIT_DEVICE_CONNECT)->SetWindowText("掉线");
			StopFlag=1;
		
			for(int i=0;i<10;i++)
			{
				Sleep(1000);
				VCI_BOARD_INFO binfo[2];
				int temp=VCI_FindGyUsbDevice(binfo);
				if(temp==1)
				{
					dlg->GetDlgItem(IDC_EDIT_DEVICE_CONNECT)->SetWindowText("在电脑,请启动设备");//重新调用opendevice函数即可启动。
					
				}
				else 
					dlg->GetDlgItem(IDC_EDIT_DEVICE_CONNECT)->SetWindowText("设备不在电脑中");
...
void CDemoCANDlg::OnCheckCanrxEn() 
{
	...
	if(m_bCanRxEn)
	{
		...
		StopFlag=1;
	GetDlgItem(IDC_EDIT_DEVICE_CONNECT)->SetWindowText(" ");
