...
BOOL CPciCanTestDlg::InitializeCan()
{
    ...
    pConfig->CanRx_IER=1;
	//初始化通道1
	if(!VCI_InitCAN(DEV_PCICAN200,0,0,pConfig))
		return FALSE;
	//初始化通道2
    if(!VCI_InitCAN(DEV_PCICAN200,0,1,pConfig))
		return FALSE;
	return TRUE;
}
/***************************************************************************
...
void CPciCanTestDlg::OnButtonSend() 
{
	...
	//调用动态库函数发送
	if(!VCI_Transmit(DEV_PCICAN200,0,m_comboCanIndex.GetCurSel(),pSend))
...
DWORD CPciCanTestDlg::Recieve()
{
	...
	while(1)
	{
		...
		{
			//接收
			dLength=VCI_Receive(DEV_PCICAN200,0,dCanindex,pRcv);
			if(dLength==-1)
...
void CPciCanTestDlg::OnCancel() 
{
	...
	if(m_hRecieve>0)
	{
		if(!VCI_CloseDevice(DEV_PCICAN200,0))
