...
void CTwoDeviceDlg::OnButtonOpen() 
{
	...
	{
		DWORD Reserved=0;
		if(VCI_OpenDevice(DEV_USBCAN,nDevice,Reserved)!=1)
		{
		...
		if(VCI_InitCAN(DEV_USBCAN,nDevice, 0,InitInfo)!=1)	//can-0
		{
		...
		if(VCI_StartCAN(DEV_USBCAN,nDevice, 0)!=1)	//can-0
		{
...
DWORD CTwoDeviceDlg::ReceiveProcess()
{
	while(1)
	{
		...
		for(int i=0;i<m_nDeiviceNum;i++)
		{
	        ...
	        NumValue=VCI_Receive(DEV_USBCAN,i,0,pCanObj);
			for(int j=0;j<NumValue;j++)
			{
				CString strSeq;
				strSeq.Format("%d",m_Seq);
				CString strDeviceNum;
				strDeviceNum.Format("%s(%d)",m_ProductSn[i],i);
...
void CTwoDeviceDlg::OnButtonSend() 
{
	...
	if(1!=VCI_Transmit(DEV_USBCAN,DeviceNum,0,&sendbuf))
	{
		AfxMessageBox("发送失败！");
	}
}
