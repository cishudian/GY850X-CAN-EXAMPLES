...
Begin VB.Form DeviceNet 
   Caption         =   "VB Example  GY7841 PCI-CAN"
   ClientHeight    =   6945
   ClientLeft      =   2490
   ClientTop       =   1740
   ClientWidth     =   11115
...
Private Sub BUTTON_INIT_CAN_Click()
    ...
    temp = VCI_InitCAN(6, 0, 0, pInitCan) '初始化0通道
...
num = VCI_Receive(6, 0, 0, pReceiveData(0)) 'CAN0通道
...
pSend.Data(6) = 38
pSend.Data(7) = 39
num = VCI_Transmit(6, 0, 0, pSend)  'send CAN1通道
i = LIST_INFO.ListCount
If num <> 0 Then
        str = Format(Now, "yyyy-mm-dd  hh:mm:ss") + vbTab + "CAN0 Send Ok: " + "  "
        For i = 0 To 3
...
Else
    LIST_INFO.List(i) = str1 + "CAN0 Send Error"
End If
