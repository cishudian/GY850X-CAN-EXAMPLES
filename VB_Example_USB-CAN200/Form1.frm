...
Private Sub BUTTON_INIT_CAN_Click()
    ...
    pInitCan.Mode = 0  ' 0~正常模式， 1~自收发模式
    pInitCan.Filter = 0
    ...
    temp = VCI_InitCAN(3, 0, 0, pInitCan) '初始化0通道
    
 
    temp = VCI_InitCAN(3, 0, 1, pInitCan) '初始化0通道
...
num = VCI_Receive(3, 0, 0, pReceiveData(0)) 'CAN0通道
...
pSend.Data(6) = 38
pSend.Data(7) = 39
num = VCI_Transmit(3, 0, 1, pSend)  'send CAN1通道
i = LIST_INFO.ListCount
If num <> 0 Then
