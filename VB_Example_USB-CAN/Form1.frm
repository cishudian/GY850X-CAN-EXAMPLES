...
Begin VB.Form DeviceNet 
   Caption         =   "VB Example  GY8507 USB-CAN Test"
   ClientHeight    =   6945
   ClientLeft      =   2490
   ClientTop       =   1740
   ClientWidth     =   11115
...
Private Sub BUTTON_INIT_CAN_Click()
    ...
    pInitCan.Mode = 1  '1~自收发模式
    pInitCan.Filter = 0
    pInitCan.kCanBaud = 15 '1000kbps
    pInitCan.Timing0 = 0  '0x00
    pInitCan.Timing1 = 20 '0x14
    pInitCan.AccCode = 1
    pInitCan.AccMask = -1 '0xFFFFFFFF
    pInitCan.CanRx_IER = 1 'Enable CAN Receive
    temp = VCI_InitCAN(2, 0, 0, pInitCan)
      If temp = 1 Then
      
       temp = VCI_StartCAN(2, 0, 0)
...
num = VCI_Receive(2, 0, 0, pReceiveData(0)) 'CAN

 For n = 0 To (num - 1)
        str = Format(Now, "yyyy-mm-dd  hh:mm:ss") + vbTab + "CAN Receive" + "    "
        For i = 0 To 3
...
pSend.Data(6) = 38
pSend.Data(7) = 39
num = VCI_Transmit(2, 0, 0, pSend)  'send CAN
i = LIST_INFO.ListCount
If num <> 0 Then
        str = Format(Now, "yyyy-mm-dd  hh:mm:ss") + vbTab + "CAN Send Ok: " + "  "
        For i = 0 To 3
...
Else
    LIST_INFO.List(i) = str1 + "CAN Send Error"
End If
...
pSend.Data(6) = 54
pSend.Data(7) = 55
num = VCI_Transmit(2, 0, 0, pSend) 'send CAN1通道
i = LIST_INFO.ListCount
If num <> 0 Then
        str = Format(Now, "yyyy-mm-dd  hh:mm:ss") + vbTab + "CAN Send Ok: " + "  "
        For i = 0 To 3
